...
def _aliases(artist_id):
  ...
  result.extend([str(x[0]) for x in con.execute(f'''select t from artist_member where t={artist_id}''')])
  result.extend([str(x[0]) for x in con.execute(f'''select t from artist_member where f={artist_id}''')])
  return ', '.join(result)
...
def releases_on_label(label_id):
  result = []
  result.extend(con.execute(f'''
...
def walk(
  ...
  # ignore massive labels like columbia
  if not mode_artist and root_id in [1818, 1866, 2311, 8377]:
    return

  if mode_artist:
    releases_count = len(releases_of_artist(root_id))
  else:
    releases_count = len(releases_on_label(root_id))

  if releases_count > 200:
    return
  ...
  if mode_artist:
    frontier = labels_of_artist(root_id)
  else:
    frontier = artists_on_label(root_id)
...
try:
  walk_start(3, True, (15725, None, None))
